...
def passGen(num):
    pas = random.sample(words, num)
    return "".join(pas)


def rep_with_upper(str):
    random_letter = random.randint(0, len(str))
    newPass = list(str)
    newPass[random_letter] = newPass[random_letter].upper()
    finalPassword = "".join(newPass)
    return finalPassword


def swap_letters(modified):
    newModified = list(modified)
    newModified[0], newModified[-2] = newModified[-2], newModified[0]
    newModified[1], newModified[-1] = newModified[-1], newModified[1]
    finalPassword = "".join(newModified)
    print("Final version of the password:", finalPassword)
    return finalPassword


def search_letter(string, letter):
    print("Searching for letter 'C' ...\nThe result is ")
...
while True:
    ...
    if user_number > 7 or user_number < 3:
        print("Invalid value!")
    else:
        password = passGen(user_number)
        print("Initial password is: ", password)
        upperCased = rep_with_upper(password)
        swapped = swap_letters(upperCased)
        break

print(search_letter(swapped, 'c' or 'C'))
